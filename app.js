  ...
  return `
    <nav class="details-side-rail" aria-label="Site navigation">
      <div class="details-rail-main">
        <a href="index.html" aria-label="Home" data-nav-label="Home"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 11.2 12 4l9 7.2v8.1a1.7 1.7 0 0 1-1.7 1.7h-4.6v-6.2H9.3V21H4.7A1.7 1.7 0 0 1 3 19.3v-8.1Zm2 1v6.8h2.3v-6.2h9.4v6.2H19v-6.8l-7-5.6-7 5.6Z"/></svg></a>
...
function profileMenuHtml(buttonClass = "icon-btn profile-btn", menuClass = "") {
  ...
  return `
    <div class="${className}">
      <button class="${buttonClass}" data-profile-toggle type="button" aria-label="Open profile settings" aria-expanded="false" data-nav-label="Profile">${profileButtonContentHtml()}</button>
...
function profileButtonContentHtml() {
  return `<img class="profile-toggle-avatar" data-profile-toggle-avatar src="${escapeAttr(profileAvatarUrl())}" alt="" decoding="async"><span class="profile-toggle-fallback">${profileInitials()}</span>`;
}

function syncProfileToggleButtons() {
  document.querySelectorAll("[data-profile-toggle]").forEach((button) => {
    button.innerHTML = profileButtonContentHtml();
    const avatar = button.querySelector("[data-profile-toggle-avatar]");
